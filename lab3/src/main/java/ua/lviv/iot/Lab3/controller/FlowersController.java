...
public class FlowersController {
    ...
    @GetMapping(path = "/{id}")
    public final Flower getFlower(final @PathVariable("id") Integer flowerId) {
        return flowers.get(flowerId);
    }

    @PostMapping(produces = {MediaType.APPLICATION_JSON_VALUE})
    public final Flower createFlower(final @RequestBody Flower flower) {
    ...
    public final ResponseEntity<Object> deleteFlower(final @PathVariable("id") Integer flowerId) {
        HttpStatus status = flowers.remove(flowerId) == null ? HttpStatus.NOT_FOUND : HttpStatus.OK;
        return ResponseEntity.status(status).build();
    }

    @PutMapping(path = "/{id}")
    public final Flower updateFlower(final @PathVariable("id") Integer flowerId, final @RequestBody Flower flower) {
